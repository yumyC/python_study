...
def create_demo_employees():
    ...
    for emp_data in DEMO_EMPLOYEES:
        # 检查员工是否已存在
        existing_emp = Employee.query.filter_by(username=emp_data["username"]).first()
        if existing_emp:
            print(f"员工 {emp_data['username']} 已存在，跳过创建")
            continue
        
        # 获取岗位和角色
        position = Position.query.filter_by(code=emp_data["position_code"]).first()
        role = Role.query.filter_by(code=emp_data["role_code"]).first()
...
def create_additional_positions():
    """
    创建更多的岗位数据，丰富组织架构
    """
    print("创建额外的岗位数据...")
    ...
    # 获取现有的部门岗位
    tech_director = Position.query.filter_by(code="TECH_DIR").first()
    hr_director = Position.query.filter_by(code="HR_DIR").first()
    
    if not tech_director or not hr_director:
        print("基础岗位不存在，请先初始化数据库")
        return
    ...
    for pos_data in additional_positions:
        # 检查岗位是否已存在
        existing_pos = Position.query.filter_by(code=pos_data["code"]).first()
        if existing_pos:
            continue
...
def main():
    ...
    主函数：创建所有演示数据
    """
    print("开始创建 CRM 系统演示数据 (Flask 版本)...")
    print("=" * 50)
    ...
    with app.app_context():
        try:
            ...
            if not admin_user:
                print("❌ 数据库尚未初始化，请先初始化数据库")
                return
            
            # 创建演示数据
            create_additional_positions()
            create_demo_employees()
            create_demo_work_logs()
