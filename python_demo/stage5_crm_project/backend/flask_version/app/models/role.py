...
class Role(BaseModel):
    ...
    def get_permissions(self):
        ...
        Returns:
            dict: 以菜单路径为键，权限列表为值的字典
        """
        permissions = {}
        
        for permission in self.menu_permissions:
            if permission.menu:
                permissions[permission.menu.path] = permission.permissions
    ...
    def has_permission(self, menu_path, permission_type):
        ...
        # 通过菜单路径查找菜单
        menu = Menu.query.filter_by(path=menu_path).first()
        if not menu:
            return False
        
        permission = self.menu_permissions.filter_by(menu_id=menu.id).first()
    ...
    def to_dict(self):
        ...
        # 添加关联信息
        data['employee_count'] = self.employees.count()
        data['permissions'] = self.get_permissions()
        data['menu_count'] = self.menu_permissions.count()
